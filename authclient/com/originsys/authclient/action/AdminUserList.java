...
/**
 auth:boy 2013-7-16
   描述：管理员用户列表
 */
public class AdminUserList extends BaseAction implements IGet {
	...
	public DataAndView<Map> execute(RequestAction ra) throws Exception {	    
		...
		int totalnum=ApiUtil.getService().getUserListCount(user);
		...
		List<UserInfo> userlist=ApiUtil.getService().getUserList(param);
