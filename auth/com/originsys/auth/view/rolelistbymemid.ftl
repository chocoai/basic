...
function delinfo(){
			...
			$.post("${_servlet_url!''}/auth.memberrole.delete",queryString,
...
function changeState(obj){
	...
	if(role_id==""){
		...
		$("#mem_state").val(obj);		
		var queryString=$("#hidform").formSerialize();
			$.post("${_servlet_url!''}/auth.memberrole.changestate",queryString,
					function(data,textStatus){
					var jdata=jQuery.parseJSON(data);
					if(jdata.success=="0"){
						alert("禁用/解除禁用失败！");
...
function addRole(){	
	var arr=window.showModalDialog("${_servlet_url!''}/manager.user.rolelistdialog?mem_id=${block.mem_id!''}","","dialogWidth:500px;dialogHeight:500px");
	if(arr!=null&&arr!=undefined){
		$("#role_id").val(arr);
		var queryString=$("#hidform").formSerialize();
			$.post("${_servlet_url!''}/auth.memberrole.insert",queryString,
					function(data,textStatus){
					var jdata=jQuery.parseJSON(data);
					if(jdata.success=="0"){
						alert("保存失败！");
...
function modifyTypeInfo(type){
	window.showModalDialog("${_servlet_url!''}/"+type+"?mem_id=${block.mem_id!''}","","dialogWidth:500px;dialogHeight:500px");
}
</script>
...
<div >
	...
	<form name="moduleform" id="moduleform" action="${_servlet_url!''}/manager.user.rolelist" method="post">
	...
	<div  id="buttons" style="text-align:right">
		[#if access.canDo(user,'auth.memberrole.insert')]
		<button type="button" onclick="addRole()">添加角色</button>[/#if]
		[#if access.canDo(user,'auth.memberrole.delete')]
		<button type="button" onclick="doDelete()">删除角色</button>[/#if]
		[#if access.canDo(user,'auth.memberrole.changestate')]
		<button type="button" onclick="changeState('3')">禁用</button>
		<button type="button" onclick="changeState('1')">解除禁用</button>[/#if]		
		<button type="button" onclick="window.location='${_servlet_url!''}/auth.admin.userlist'">返回</button>
	</div>
	<div id="pager1"></div>
