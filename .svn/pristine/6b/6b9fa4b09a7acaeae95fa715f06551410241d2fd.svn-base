...
$(function(){
	jQuery("#clist1").jqGrid({
	   	url:'${_servlet_url!''}/safecheck.survey.editlistjson',
		datatype: "json",
		width:600,
		height:300,
	   	colNames:[
	...
	jQuery("#gridReload").click(function() {
		var url="${_servlet_url!''}/safecheck.survey.editlistjson?"
	...
	$("#updateInfo").click(function(){
		...
		if (id) { 
			...
			var url="${_servlet_url!''}/safecheck.survey.forupdate?survey_id="+ret.survey_id+"&&building_id="+ret.building_id;
	...
	$("#selectInfo").click(function(){
		...
		if (id) { 
			var ret = jQuery("#clist1").jqGrid('getRowData',id);
			window.location="${_servlet_url!''}/safecheck.survey.detail?survey_id="+ret.survey_id+"&&building_id="+ret.building_id;	
		}else{
			alert("请选择一条记录！");
		}
	});		
	...
	$("#deleteInfo").click(function(){
		...
		if (id) {
			...
			if(flag){
				$.post("${_servlet_url!''}/safecheck.survey.delete?building_id="+ret.building_id+"&&survey_id="+ret.survey_id,"",function(data,textStatus){
...
function emptiedAndSubmit(){
	...
	$("#survey_date").val("");
	$("#survey_type").val("");
    jQuery("#clist1").jqGrid('setGridParam',{url:encodeURI("${_servlet_url!''}/safecheck.survey.editlistjson"),page:1}).trigger("reloadGrid");
}
