...
/**
 auth:zhanglf 2014-6-10
   描述：楼幢检查结果新增，修改基本信息表，新增楼幢普查结果表，地基基础表，现场调查场地环境
 */
public class BuildingSafeInsert extends BaseAction implements IData{
...
//		tBuilding.setDangerous_type_jd(ra.getParameter("dangerous_type_jd"));//String:危房类型-鉴定 整栋危楼1 局部危楼2
		...
		String info_id=UUIDshort.get();//主键id
		/**楼幢普查结果信息*/
		...
		try{
			...
			//修改日志
			ra.operate.setOperateModule("新增检查信息：楼幢编号"+building_id);
			ra.operate.setOperateContent(opcontent.toString());
			ra.operate.setOperateType("新增");
