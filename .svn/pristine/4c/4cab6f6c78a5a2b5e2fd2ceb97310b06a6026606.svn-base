...
$(function(){
	...
	jQuery("#clist1").jqGrid({
	   	url:'${_servlet_url!''}/auth.admin.userlistjson',
		datatype: "json",
		width:600,
		height:300,
	   	colNames:[
	...
	jQuery("#gridReload").click(function() {
		var url="${_servlet_url!''}/auth.admin.userlistjson"
	...
	$("#addInfo").click(function(){
		var url="${_servlet_url!''}/auth.admin.userInfoforadd";		
		window.showModalDialog(url,"","dialogWidth:550px;dialogHeight:550px;center:1;");
		jQuery("#clist1").trigger('reloadGrid');
	});
...
function toRoles(){
	...
	if (id) { 
		var ret = jQuery("#clist1").jqGrid('getRowData',id);
		window.location="${_servlet_url!''}/auth.user.rolelist?mem_id="+ret.mem_id+"&user_name="+encodeURI(ret.family_name);
	}
	else
		warn("请选择一条记录修改！");
}
...
function checkUser(state){
	var id = jQuery("#clist1").jqGrid('getGridParam','selrow');
	var ret = jQuery("#clist1").jqGrid('getRowData',id);
	$.post("${_servlet_url!''}/auth.admin.userInfocheck?mem_id="+ret.mem_id+"&state="+state,"",function(data,textStatus){
		var jdata=jQuery.parseJSON(data);
		if(jdata.success==1){
			jQuery("#clist1").trigger('reloadGrid');
			$("#delete-dialog").dialog("close");
...
<div  id="buttons" style="text-align:right;margin-bottom:8px;">
		...
		<button type="button" align="right" onclick="checkUser('0');">禁用</button>
		<button type="button" align="right" onclick="checkUser('1');">解除禁用</button>
...
<div id="dialogresetPass" title="修改用户密码" style="display:none">
	<form class="cmxform" style="margin:0" action="${_servlet_url!''}/auth.resetpass" method="post" id="resetForm">
