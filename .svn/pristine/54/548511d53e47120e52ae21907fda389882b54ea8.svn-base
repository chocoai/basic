...
<form class="cmxform" style="margin:0" action="${_servlet_url!''}/safemanage.safeassessors.update1" method="post" id="updateForm">
	<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#dee2e3" style="line-height:30px;">
		<input type="hidden" name="mem_id" id="mem_id" value="${result.mem_id!''}"/>		
			...
			<tr>
					<td class="rightpad">手写签名</td>
					<td class="leftpad">
						<input type="hidden" name="signature" id="signature" value="${result.signature!''}"/>
						<div id="imguploaddiv1"></div>
						<script language="javascript" type="text/javascript">
							var so = new SWFObject('${_share_file_url!''}/resource/jsp/imgupload.swf', "imgupload1", "125", "60", "9", "#FFffff");//imgupload是控件ID,如有多个ID不可重复
