...
$(function(){
	jQuery("#clist1").jqGrid({
	   	url:'${_servlet_url!''}/safecheck.noticewarnlistjson',
		datatype: "json",
		width:600,
		height:300,
	   	colNames:[
	...
	jQuery("#gridReload").click(function() {
		var url="${_servlet_url!''}/safecheck.noticewarnlistjson?"
...
function emptiedAndSubmit(){
	...
	$("#dangerous_level").val("");
	$("#over_grade").val("");
    jQuery("#clist1").jqGrid('setGridParam',{url:encodeURI("${_servlet_url!''}/safecheck.noticewarnlistjson"),page:1}).trigger("reloadGrid");
}
