...
<form class="cmxform" style="margin:0" action="${_servlet_url!''}/safemanage.tBuildingUnitupdate" method="post" id="updateForm">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" align="center" style="table-layout:fixed;line-height:30px;">
			<col width="150px"><col />
			<tr>
				<td class="rightpad"><font color="red">*&nbsp;</font>安全管理单位名称</td>
				<td class="leftpad">
					<input type="text" name="unit_name" id="unit_name" value="${result.unit_name!''}" size="40"/>
				</td>
			</tr>
			<tr>
				<td class="rightpad"><font color="red">*&nbsp;</font>安全管理单位类型</td>
				<td class="leftpad">
					<select name="unit_type" id="unit_type">
						<option value="">--请选择--</option>
						[#if EnumService.getEnum('safe_unit_type')?exists]
						[#list EnumService.getEnum('safe_unit_type') as enum]
						<option value="${enum.enum_value!''}" [#if "${result.unit_type!''}"=="${enum.enum_value!''}"]selected[/#if]>${enum.enum_name!''}</option>
						[/#list]
						[/#if]
					</select>
				</td>
			...
			<tr>
				...
				<td class="leftpad">
					<input type="text" name="link_man" id="link_man" value="${result.link_man!''}"  size="20"/>
				</td>
			</tr>
			<tr>
				<td class="rightpad">负责人电话</td>
				<td class="leftpad">
					<input type="text" name="link_phone" id="link_phone" value="${result.link_phone!''}" size="20"/>
				</td>
			</tr>
			<tr>
				...
				<td class="leftpad">
					<select name="city_district" id="city_district">
						...
						<option value="${enum.enum_value!''}" [#if "${result.city_district!''}"=="${enum.enum_value!''}"]selected[/#if]>${enum.enum_name!''}</option>
						[/#list]
						[/#if]
					</select>
				</td>
			...
			<tr>
				...
				<td class="leftpad">
					<textarea name="unit_desc" id="unit_desc" rows="3" cols="40">${result.unit_desc!''}</textarea>
				</td>
			</tr>
			<tr>
