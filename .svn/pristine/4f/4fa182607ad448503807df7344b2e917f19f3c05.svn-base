	    ...
	    function createInfoWindow(feature){
      	// 获取西南角坐标（left，bottom）/房屋点坐标smx,smy
			...
			if(smuserid<0)//针对新建房屋操作
			 url+="report.forinsert1s?smuserid="+smuserid+"&left="+left+"&bottom="+bottom+"&address="+address;
			else//针对已有房屋操作
			 url+="report.forinserts?smuserid="+smuserid+"&left="+left+"&bottom="+bottom+"&address="+address;			
			...
			// 初始化一个弹出窗口，当某个地图要素被选中时会弹出此窗口，用来显示选中地图要素的属性信息
			 ...
			 if($("#messinsert")){
		     $("#messinsert").bind("click",function() {  
		       var b=url+"report.forinserts?smuserid="+smuserid+"&left="+left+"&bottom="+bottom+"&address="+address;
