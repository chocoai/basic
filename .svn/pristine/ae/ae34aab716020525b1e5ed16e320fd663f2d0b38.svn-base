...
<div id="tooldiv" style=" margin-top:5px;margin-bottom:0px;"><button class="button" type="button" id="chooseHouse">选取房屋</button>
...
$(function(){	
	 map = new FMapLib.FMap("map","","${user.region_id!''}",6);
      	...
      	// 获取西南角坐标（left，bottom）/房屋点坐标smx,smy
			...
			var url="safecheck.";						
			   var  contentHTML2 = "<div style='font-size:.8em; opacity: 1; line-height:30px; overflow-y:hidden;'>"
			    ...
			    contentHTML2 +="<p  style='text-align:center;'><input type='submit' name='Submit' id='messfind' value='查看健康手册' " +
			    		"style='background:url(/gis/FMapLib/theme/images/button_bj.jpg) repeat-x; width:100px; height:35px; border:1px #cbe5ff solid; " +
			    		"line-height:0px; font-family:微软雅黑; font-size:14px; color:#005ebc;'/></p>";
				 }
			    contentHTML2 += "</div>";
			// 初始化一个弹出窗口，当某个地图要素被选中时会弹出此窗口，用来显示选中地图要素的属性信息
			 var   popup = new MapLib.Popup.FramedCloud("popwin",
			   ...
			   map.addPopup(popup);	
			   url+="healthrecorddetail?building_id="+smuserid;	
