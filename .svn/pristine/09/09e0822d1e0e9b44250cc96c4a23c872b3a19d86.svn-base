...
/**
 auth:boy 2014-7-28
   描述：6.	房屋用途统计图（柱状图）
按照房屋用途标准字段“住宅、非住宅、商业服务、商住、其它”来进行统计，展示各区县各房屋用途的普查数量。
 */
public class SurveyUseDesginTJ  extends BaseAction implements IGet{
	...
	public DataAndView execute(RequestAction arg0) throws Exception {
		...
		/**获取权属状况的枚举列表*/
		List<EnumValue> qslist=EnumService.getInstance().getEnum("sjyt");
		...
		for(int i=0;i<qslist.size();i++){
			...
			/**一种权属状况各个区县的统计结果*/
			List tmpResult=(List)sc.queryForList("Safecheck.getSurveyUseDesginTj",qsid);
		...
		List tmpIDsResult=(List)sc.queryForList("Safecheck.getSurveyUseDesginMembersTj");
