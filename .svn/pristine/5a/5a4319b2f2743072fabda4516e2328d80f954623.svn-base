...
//初始化地图
	 ...
	 $("#safegradetheme").click(function(){
      var typevalue=$("#survey_type").val();
      if(!!typevalue)   
      BS= new FMapLib.BuildingSafeGradeRange(map,typevalue); //生成安全等级专题图
      else{
       BS= new FMapLib.BuildingSafeGradeRange(map); //生成安全等级专题图
      }
  ...
  $("#printmap").hover(function () {
	...
	//	$("#"+map.legendId).css("display", "none");
	//});
	//显示图例
	//$("#showlegend").click(function(){
	//   $("#"+map.legendId).css("display", "block");
	//});
	//打印地图
...
//	var mountArray=[{"A级":183,"B级":13,"C级":15,"D级":140}];//待修改为AJAXS请求服务端返回ＪＳＯＮ数组形式。参考sql：select count(*),t.健康完损等级 from st_ridrgn_jkda_p t group by  t.健康完损等级
	...
	html+="<td width='90' height='7'align='left'>无问题房屋</td></tr>";
	html+="<tr><td width='15'  height='7' bgcolor='"+color[3]+"'></td>";
	html+="<td width='90' height='7'align='left'>有问题房屋</td></tr>";
	html+="</table></div>" ; 
...
<button  class="button" id="safegradetheme">生成专题图</button>&nbsp<button  class="button" id="cleartheme">清除专题图</button>&nbsp<button  class="button" id="printmap">打印地图</button>
</div>
