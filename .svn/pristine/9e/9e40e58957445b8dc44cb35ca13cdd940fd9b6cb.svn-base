...
<form action="${_servlet_url!''}/safeauth.report.update" id="addForm" method="post">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#dee2e3" style="line-height:30px;">
		<tr>
			...
			<td class="td13" colspan="3"><input type="text" name="building_address" id="building_address" size="60" value="${result.building_address!''}"></td>
			<td class="td12"><font color="red">*&nbsp;</font>所属区域：</td>
			<td class="td13">
				<select id="building_region" name="building_region">
					<option value="">------请选择------</option>
					[#list EnumService.getEnum('xzqh') as enum]
					<option value="${enum.enum_value!''}" [#if "${result.building_region!''}"=="${enum.enum_value!''}"]selected[/#if]>${enum.enum_name!''}</option>
					[/#list]
				</select>
			</td>
		</tr>
		<tr>
			...
			<td class="td13"><input type="text" name="entrust_user" value="${result.entrust_user!''}"></td>
			<td class="td12">联系人：</td>
			<td class="td13"><input type="text" name="linkman" value="${result.linkman!''}"></td>
			<td class="td12">联系电话：</td>
			<td class="td13"><input type="text" name="linktel" value="${result.linktel!''}"></td>			
		</tr>
		<tr>
			...
			<td class="td13"><input type="text" name="jdmember" id="jdmember" value="${result.jdmember!''}"></td>
			<td class="td12"><font color="red">*&nbsp;</font>鉴定时间：</td>
			<td class="td13"><input type="text" id="jd_date" name="jd_date" [#if result.jd_date?exists]value="${result.jd_date?string('yyyy-MM-dd')}"[#else]value=""[/#if]></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<div class="file-box">
				<input type="hidden" name="jd_report" id="jd_report" class='txt' value="${result.jd_report!''}">
				<input type="text" name="jd_report1" id="jd_report1" class='txt' value="${result.jd_report!''}">
				<input type='button' class="btn" value='浏览...' />
				<input type="file" id="fileToUpload1" size="28" name="fileToUpload1" class="file" onchange="document.getElementById('jd_report1').value=this.value" >
				<input type='button' onclick="return fileUpload();" class="btn" value='点击上传' />
				<input type='button' onclick="fileDownload()" id="returnval1" name="returnval1"  class="btn"  style="margin-left:5px;[#if "${result.jd_report!''}"!='']display:inline-block[#else]display:none[/#if];" value="报告下载"/>
			</div>
			</td>
		</tr>
		<tr>
			...
			<td class="td13" colspan="5">
			<div class="file-box">
				<input type="hidden" name="jd_image" id="jd_image" class='txt'  value="${result.jd_image!''}">
				<input type="text" name="jd_image1" id="jd_image1" class='txt'  value="${result.jd_image!''}">
				<input type='button' class="btn" value='浏览...' />
				...
				<input type='button' onclick="imageDownload()"  class="btn"  id="returnval" name="returnval" style="margin-left:5px;[#if "${result.jd_image!''}"!='']display:inline-block[#else]display:none[/#if];" value="查看图片"/>
			</div>
			</td>
		</tr>
		<tr>
			...
			<td class="td13" colspan="5">
			<textarea cols="100" rows="5" name="jz_overview">${result.jz_overview!''}</textarea>
			</td>
		</tr>
		<tr>
			<td class="td12">鉴定结论：</td>
			<td class="td13" colspan="5">
			<textarea cols="100" rows="5" name="identify_conclusion">${result.identify_conclusion!''}</textarea>
			</td>
		</tr>
		<!-- tr>
