{ 
 ...
 {"cell" :[		
	    ...
	    [#if "${result.info_state!''}"=="8"][#assign info_state="审核通过"][/#if]
				...
				"${result.building_id!''}"
				,"${result.survey_id!''}"
				,"<a href='javascript:void(0);' onclick=open('${_servlet_url!''}/safecheck.survey.detail?building_id=${result.building_id!''}&&survey_id=${result.survey_id!''}','_blank','depended=yes,top='+(window.screen.height-30-600)/2+',left='+(window.screen.width-10-1000)/2+',width=1000,height=600,menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=yes');>${result.building_address!''}</a>"
				,"${yt!''}"
