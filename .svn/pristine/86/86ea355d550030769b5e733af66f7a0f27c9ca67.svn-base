//var org = "http://192.168.0.12:8090";// GIS主服务器地址
//var DemoURL = {
//		fangchan : org + '/iserver/services/map-XiuGai803/rest/maps/1403010200'
		
//	};
//楼栋查询地图上marker展示
function markerShow(building_id){
	...
	if(building_id==""){
		...
		// 解决多余逗号问题
		//var smuserid=building_id+"0";
		var smuserid = building_id.substring(0,building_id.lastIndexOf(","))
	 ...
	 */
	 t_buildingMarkerQuery=function(marker){
			...
			$
					.ajax({
						url : 'safecheck.buildingcommonsurvey',
						cache : false,
						async : false,// 同步
						dataType : 'json',
						data : {
						...
						success : function(item, textStatus, jqXHR) {
							if (textStatus == 'success') {
								...
								contentHTML +="<p  style='text-align:center;'>" +
										...
										"onclick=window.open('safecheck.survey.detail?isgis=1&building_id="	+ item.building_id+"','_blank','depended=yes,top='+(window.screen.height-30-500)/2+',left='+(window.screen.width-10-900)/2+',width=900,height=500,menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=yes') " +
...
//
//			         	contentHTML +="<form>"
//			         		        +"<p align='center'><input type='button' id='buildinginfo' value='详细信息'  onclick=window.open('safecheck.survey.detail?isgis=1&building_id="
