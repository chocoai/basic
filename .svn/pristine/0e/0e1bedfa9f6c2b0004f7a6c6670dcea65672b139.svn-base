<!--[#list map.relist as relist]
	...
	[#list relist.TYPELIST as typelist]
		...
		[#list typelist.JDLIST as jdlist]
			区县：${jdlist.DISTRICT!''},套数：${jdlist.COUNT!''}<br/>
		[/#list]
	[/#list]
[/#list]-->
...
<script type="text/javascript">
	...
	[#if EnumService.getEnum('chtjlx')?exists]
	   [#list EnumService.getEnum('chtjlx') as enum]
				[#if "${relist.SURVERTYPE!''}" == "${enum.enum_value!''}"]
					strXML="<chart palette='2' caption='各区县测绘' subcaption='按季度${enum.enum_name!''}房屋套数统计' showvalues='0' xAxisName='区县' yAxisName='套数' restoreBtnBorderColor='A2A3A0' formBtnBorderColor='A2A3A0' showFormBtn='0' showRestoreBtn='0' chartRightMargin='30' baseFontSize='12' showAboutmenuItem='0'>";
				[/#if]
		[/#list]
	[/#if]
   ...
   var strDataset="";
	...
	strDataset+="<dataset id='${typelist.YEARJD!''}' seriesName='${typelist.YEARJD!''}'>";
		[#list typelist.JDLIST as jdlist]
			strDataset+="<set id='${jdlist.DISTRICT!''}${jdlist.COUNT!''}' value='${jdlist.COUNT!''}' allowDrag='0'/>";
		[/#list]
		strDataset+="</dataset>";
	[/#list]
	//拼接样式设置
