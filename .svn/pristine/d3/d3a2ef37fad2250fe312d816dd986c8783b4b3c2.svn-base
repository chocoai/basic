<!--[#list map.relist as relist]
	...
	[#list relist.typeList as typelist]
		...
		[#list typelist.districtList as district]
			区县：${district.DISTRICT!''}----套数：${district.COUNT!''}<br/>
		[/#list]
	[/#list]
[/#list]-->
	...
	[#if EnumService.getEnum('chtjlx')?exists]
	   [#list EnumService.getEnum('chtjlx') as enum]
				[#if "${relist.SURVERTYPE!''}" == "${enum.enum_value!''}"]
					strXML="<chart palette='2' caption='各区县测绘' subcaption='按年${enum.enum_name!''}房屋套数统计' showvalues='0' xAxisName='区县' yAxisName='套数' restoreBtnBorderColor='A2A3A0' formBtnBorderColor='A2A3A0' showFormBtn='0' showRestoreBtn='0' chartRightMargin='30' baseFontSize='12' showAboutmenuItem='0'>";
				[/#if]
		[/#list]
	[/#if]
   ...
   var strDataset="";
	...
	strDataset+="<dataset id='${typelist.YEAR!''}' seriesName='${typelist.YEAR!''}年'>";
		[#list typelist.districtList as dlist]
			strDataset+="<set id='${dlist.DISTRICT!''}${dlist.COUNT!''}' value='${dlist.COUNT!''}' allowDrag='0'/>";
		[/#list]
		strDataset+="</dataset>";
	[/#list]
	//拼接样式设置
