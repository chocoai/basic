<!--[#list map.relist as relist]
	...
	[#list relist.TYPELIST as typelist]
		季度：${typelist.YEARJD!''}<br/>
		[#list typelist.JDLIST as jdlist]
			区县：${jdlist.DISTRICT!''},建筑面积：${jdlist.AREA!''}<br/>
		[/#list]
	[/#list]
[/#list]-->
...
<script type="text/javascript">
	...
	[#if EnumService.getEnum('chtjlx')?exists]
	   [#list EnumService.getEnum('chtjlx') as enum]
				[#if "${relist.SURVERTYPE!''}" == "${enum.enum_value!''}"]
					strXML="<chart palette='2' caption='各区县测绘' subcaption='按季度${enum.enum_name!''}房屋建筑面积统计' showvalues='0' xAxisName='区县' yAxisName='面积（单位：平方米）' restoreBtnBorderColor='A2A3A0' formBtnBorderColor='A2A3A0' showFormBtn='0' showRestoreBtn='0' chartRightMargin='30' baseFontSize='12' showAboutmenuItem='0'>";
				[/#if]
		[/#list]
	[/#if]
   ...
   var strDataset="";
	[#list relist.TYPELIST as typelist]
	strDataset+="<dataset id='${typelist.YEARJD!''}' seriesName='${typelist.YEARJD!''}'>";
		[#list typelist.JDLIST as jdlist]
			strDataset+="<set id='${jdlist.DISTRICT!''}${jdlist.AREA!''}' value='${jdlist.AREA!''}' allowDrag='0'/>";
		[/#list]
		strDataset+="</dataset>";
	[/#list]
	//拼接样式设置
