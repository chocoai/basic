...
/**
 auth:boy 2014-5-8
   描述:安全普查结果管理列表
 */
public class BuildingSafeManageList extends BaseAction implements IGet{
...
//		}
		...
		if(region!=null&&!"".equals(region)){
			tBuilding.setBuiliding_region(region);//该用户有管理的区域
		}else{
		...
		//获取总条数
		int totalnum=(Integer)sc.queryForObject("Safecheck.getBuildingSafeManageListCount", tBuilding);
		//获得总页数
		...
		//查询结果
		List<TBuilding> resultList=(List<TBuilding>)sc.queryForList("Safecheck.getBuildingSafeManageList", param);
