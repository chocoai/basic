...
public class DistrictProjectSurveyJdTj extends BaseAction implements IGet{
	/**
	 * 类说明：各区县按季度预实测套数统计
	 * @创建时间：2014年4月28日
	 * @作者：zhanglf
...
//		System.out.println("type:"+type+"--surverStartDate:"+surverStartDate+"--surverEndDate:"+surverEndDate+"--jd1:"+jd1+"---jd2---"+jd2);
		...
		if(null!=surverStartDate&&!"".equals(surverStartDate)&&null!=surverEndDate&&!"".equals(surverEndDate)){
			...
			List yearResult=(List)sc.queryForList("Realtygis.getDistrictProjectSurveyJdtj", param);//从数据库中按季度获取各区县的各测绘类型的房屋套数
//			for(int i=0;i<yearResult.size();i++){
//				Map m=(Map) yearResult.get(i);
//				System.out.println("----count----"+m.get("COUNT")+"----SURVERYEAR--"+m.get("SURVERYEAR")+"---SURVERTYPE---"+m.get("SURVERTYPE")+"----jd---"+m.get("SURVERJD")+"---district---"+m.get("DISTRICT"));
//			}
...
//						System.out.println("-----count---"+m2.get("COUNT")+"-----district---"+m2.get("DISTRICT"));
//					}
//				}
//			}
