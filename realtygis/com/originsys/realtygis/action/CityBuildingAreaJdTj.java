...
public class CityBuildingAreaJdTj extends BaseAction implements IGet{
	/**
	 * 类说明：全市测绘按季度房屋建筑面积统计
	 * @创建时间：2014年4月24日
	 * @作者：zhanglf
...
//		System.out.println("type:"+type+"--surverStartDate:"+surverStartDate+"--surverEndDate:"+surverEndDate+"--jd1:"+jd1+"---jd2---"+jd2);
		...
		if(null!=surverStartDate&&!"".equals(surverStartDate)&&null!=surverEndDate&&!"".equals(surverEndDate)){
			...
			List yearResult=(List)sc.queryForList("Realtygis.getCityBuildingAreaJdtj", param);//从数据库中按季度获取全市的各测绘类型的套数
//			for(int i=0;i<yearResult.size();i++){
//				Map m=(Map) yearResult.get(i);
//				System.out.println("----area----"+m.get("AREA")+"----SURVERYEAR--"+m.get("SURVERYEAR")+"---SURVERTYPE---"+m.get("SURVERTYPE")+"----jd---"+m.get("SURVERJD"));
//			}
...
//					System.out.println("----AREA----"+mmm.get("AREA")+"----yearmonth----"+mmm.get("YEARJD"));
//				}
