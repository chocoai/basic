...
public class PointsCoordinateTrans extends BaseAction implements IData{
	...
	public void execute(RequestAction ra, HttpServletResponse response)
		...
		double lat=0.0;
		try{
		if(null!=ra.getParameter("lon")&&!"".equals(ra.getParameter("lon")))
		 lon=Double.parseDouble(ra.getParameter("lon").toString());		
		if(null!=ra.getParameter("lat")&&!"".equals(ra.getParameter("lat")))
		 lat=Double.parseDouble(ra.getParameter("lat").toString());		
		//局部单线程
	synchronized (out) {	
		//out.println("开始本地转换。。");
		point2d= new Point2D(lon,lat);			
		// 经纬度投影
		...
		prjCoordSysDes, PrjCoordSys, coordSysTransParameter,
