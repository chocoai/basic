...
$(function(){	
	...
	jQuery("#clist1").jqGrid({
	   	url:'${_servlet_url!''}/safeauth.dangerous.listjson',
		datatype: "json",
		width:600,
		height:300,
	   	colNames:[
	   	...
	   	colModel:[
			...
			,{name:'wfzz_status',index:'build_struct',sortable:true,width:60}
	...
	jQuery("#gridReload").click(function() {
		var url="${_servlet_url!''}/safeauth.dangerous.listjson"
	...
	$("#noticeInfo").click(function(){
		...
		if (id) { 
			...
			if(ret.notice_state_id=="1"){
				...
				$("#jdinfo_id",$("#noticeForm")).val(ret.jdinfo_id);
				$("#notice_content").val("");
				$("#notice_file").val("");
	...
	$("#addRepairInfo").click(function(){
		var id = jQuery("#clist1").jqGrid('getGridParam','selrow');
		if (id) { 
			var ret = jQuery("#clist1").jqGrid('getRowData',id);
			var url="${_server_url!''}/portal/safecheck.repair.foradd?building_id="+ret.building_id;
			if (window.showModalDialog!=null)
				window.showModalDialog(url,"","dialogWidth:600px;dialogHeight:470px;status:no;help:no;scrolling=no;scrollbars=no");
			else
				window.open(url,"","width=600px,height=470px,menubar=no,toolbar=no,location=no,scrollbars=no,status=no,modal=yes");
		}else{
			alert("请选择一条记录！");
		}
	});
	
	$("#addZZRecords").click(function(){
		var id = jQuery("#clist1").jqGrid('getGridParam','selrow');
		if (id) { 
			var ret = jQuery("#clist1").jqGrid('getRowData',id);
			var url="${_server_url!''}/portal/safecheck.wfzz.4add?building_id="+ret.building_id+"&builiding_region="+encodeURI(encodeURI(ret.builiding_region));
			if (window.showModalDialog!=null)
				window.showModalDialog(url,"","dialogWidth:600px;dialogHeight:470px;status:no;help:no;scrolling=no;scrollbars=no");
			else
				window.open(url,"","width=600px,height=470px,menubar=no,toolbar=no,location=no,scrollbars=no,status=no,modal=yes");
...
function emptiedAndSubmit(){
	...
	$("#health_grade_jd").val("");
	$("#is_die").val("");
    jQuery("#clist1").jqGrid('setGridParam',{url:encodeURI("${_servlet_url!''}/safeauth.dangerous.listjson"),page:1}).trigger("reloadGrid");
}
...
<div  id="buttons" style="text-align:right;margin-bottom:8px;">
	...
	<button type="button" id="addInfo" align="right">危房处置</button>
	<button type="button" id="addRepairInfo" align="right" style="DISPLAY:none">增加维修记录</button>
	<button type="button" id="addZZRecords" align="right">增加整治记录</button>
</div>
<div id="pager1"></div>
...
<div id="dialog" title="危楼处置信息" style="display:none">
	<form class="cmxform" style="margin:0" action="${_servlet_url!''}/safeauth.dangerouslog.insert" method="post" id="addForm">
		...
		<input type="hidden" name="jdinfo_id" id="jdinfo_id">
