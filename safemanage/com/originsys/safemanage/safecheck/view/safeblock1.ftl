...
<div class="head">
    ...
    <div class="nav" id="nav" style="width:800px;"> 
      <ul>
        ...
        <li><a  _for="dangerhousewarn" id="dangerhousewarn" target="main" onClick="changeLeftMenu(this);">危房预警</a></li>
        [/#if]
...
<table class="frame_main" cellpadding="0" cellspacing="0">
	...
	<td id="td_left" class="bg_color" width="210" heigth="300" style="overflow:auto;" algin="top" valign="top">
			...
			<div id="menu_common" style="padding:8px;"> 
					...
					<div>
						<ul>
							<li><a id="safecheck.whing" onclick="changeTab('safecheck.whing','全景专题图')" title="全景专题图" target="main">全景专题图</a></li>
						</ul>
					</div>
		...
		<div style="padding-left:10px;font-weight:bold;font-size:14px;padding-right:5px;line-height:36px;height:36px;background-image:url(../resource/images/rebg.jpg);background-repeat:repeat-x;">
			...
			<div id="menu_member" style="padding:8px;"> 
					[#if access.canDo(user,'safecheck.safemanager.list')||access.canDo(user,'safecheck.safecensor.list')||access.canDo(user,'safecheck.safeassessors.list')]
					<h2><a>人员备案管理</a></h2>
						<div>
							<ul>
								[#if access.canDo(user,'safecheck.safecensor.list')]
								<li><a id="safecheck.safecensor.list" onclick="changeTab('safecheck.safecensor.list','安全检查员管理')" title="安全检查员管理" target="main">安全检查员管理</a></li>
								[/#if]
								[#if access.canDo(user,'safecheck.safeassessors.list')]
								<li><a id="safecheck.safeassessors.list" onclick="changeTab('safecheck.safeassessors.list','安全鉴定员管理')" title="安全鉴定员管理" target="main">安全鉴定员管理</a></li>		
								[/#if]
								[#if access.canDo(user,'safecheck.safemanager.list')]
								<li><a id="safecheck.safemanager.list" onclick="changeTab('safecheck.safemanager.list','安全管理员管理')" title="安全管理员管理" target="main">安全管理员管理</a></li>			
								[/#if]
							</ul>
						</div>
					[/#if]
					[#if access.canDo(user,'commonservice.department.editlist')]
				  	<h2><a>机构管理</a></h2>
				  		<div>
							<ul>
								<li><a id="commonservice.department.editlist?com_id=1" onclick="changeTab('commonservice.department.editlist?com_id=1','安全鉴定机构管理')" title="安全鉴定机构管理" target="main">安全鉴定机构管理</a></li>
							</ul>
						</div>
					[/#if]
			</div>
			...
			<div id="menu_login" style="padding:8px;">
				  ...
				  <div>
				  		<ul>
					    	...
					    	<li><a id="commonservice.login.forchangepassword" onclick="changeTab('commonservice.login.forchangepassword','修改密码')" title="修改密码" target="main"><span>修改密码</span></a></li>
				    	</ul>
				  </div>
			</div>
		</div>
