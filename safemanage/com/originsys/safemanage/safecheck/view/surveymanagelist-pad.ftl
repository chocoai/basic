...
$(function(){
	jQuery("#clist1").jqGrid({
	   	url:'${_servlet_url!''}/safecheck.survey.padmanagelistjson',
		datatype: "json",
		width:600,
		height:300,
	   	colNames:[
	...
	jQuery("#gridReload").click(function() {
		var url="${_servlet_url!''}/safecheck.survey.padmanagelistjson?"
...
function emptiedAndSubmit(){
	...
	$("#info_state").val("");
    jQuery("#clist1").jqGrid('setGridParam',{url:encodeURI("${_servlet_url!''}/safecheck.survey.padmanagelistjson"),page:1}).trigger("reloadGrid");
}
