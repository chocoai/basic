...
$(document).ready(function(){ 
	...
	$("#menu_healthrecord").accordion({heightStyle: "content",clearStyle: true,collapsible: true  });
	//隐藏菜单
	...
	[#if access.canDo(user,'safecheck.survey.editlist')||access.canDo(user,'safecheck.survey.managelist')||access.canDo(user,'safecheck.survey.housecounthz')||access.canDo(user,'safecheck.survey.housepoicheck')]
        ...
        [#elseif access.canDo(user,'71.news.editlist')||access.canDo(user,'72.news.editlist')||access.canDo(user,'73.news.editlist')||access.canDo(user,'74.news.editlist')||access.canDo(user,'75.news.editlist')]
        [#assign caidan="info"]  
        [#elseif access.canDo(user,'authclient.userInfolist')||access.canDo(user,'safemanage.tBuildingProjectlist')]
        [#assign caidan="member"]    
        [/#if]
	...
	function changeTab(id,title){	
	   var  idstr=id+"";   
	        if(idstr.indexOf("http")!=-1){	
	        $("#actionUrl").val("<iframe name='main' src='"+id+"' frameborder='0' style='width:100%'></iframe>");	
	        }else{
...
<div class="head">
    ...
    <div class="nav" id="nav"  style="width:800px;">
      <ul>
      	[#if access.canDo(user,'safecheck.survey.editlist')||access.canDo(user,'safecheck.survey.managelist')||access.canDo(user,'safecheck.survey.housecounthz')||access.canDo(user,'safecheck.survey.housepoicheck')]
        ...
        [/#if]
       <!-- [#if access.canDo(user,'safecheck.healthrecorddetail')||access.canDo(user,'safecheck.survey.poihealthrecord')]
        <li><a  _for="healthrecord" id="healthrecord" target="main" onClick="changeLeftMenu(this);">健康手册</a></li>
        [/#if]-->
        [#if access.canDo(user,'71.news.editlist')||access.canDo(user,'72.news.editlist')||access.canDo(user,'73.news.editlist')||access.canDo(user,'74.news.editlist')||access.canDo(user,'75.news.editlist')]
       	<li><a  _for="info" id="info" target="main" onClick="changeLeftMenu(this);">信息发布</a></li>       
        [/#if]
        [#if access.canDo(user,'authclient.userInfolist')||access.canDo(user,'safemanage.tBuildingProjectlist')]
        <li><a  _for="member" id="member" target="main" onClick="changeLeftMenu(this);">人员机构备案</a></li>
        [/#if]
...
<table class="frame_main" cellpadding="0" cellspacing="0">
	...
	<td id="td_left" class="bg_color" width="210" heigth="300" style="overflow:auto;" algin="top" valign="top">
			...
			<div id="menu_common" style="padding:8px;"> 
					...
					<div>
						<ul>
							<li><a id="safecheck.poihealthrecord" onclick="changeTab('safecheck.poihealthrecord','房屋健康手册')" title="房屋健康手册" target="main">房屋健康手册</a></li>
						</ul>
					</div>
			</div>
		</div>
			...
			<div id="menu_workflow" style="padding:8px;">
					...
					<div>
						<ul>
							...
							<li class="header"><a id="safecheck.building.choosehouse" onclick="changeTab('safecheck.building.choosehouse','安全检查录入')" title="安全检查录入" target="main">安全检查录入</a></li>
							<li class="header"><a id="safecheck.building.editlist" onclick="changeTab('safecheck.building.editlist','日常检查列表')" title="日常检查列表" target="main">日常检查列表</a></li>
							[/#if]							
							[#if access.canDo(user,'safecheck.building.managelist')]
							<li class="header"><a id="safecheck.building.managelist" onclick="changeTab('safecheck.building.managelist','日常检查审核')" title="楼幢检查审核" target="main">日常检查审核</a></li>
							[/#if]
							[#if access.canDo(user,'safecheck.accident.list')]
							<li class="header"><a id="safecheck.accident.list" onclick="changeTab('safecheck.accident.list','事故记录列表')" title="事故记录列表" target="main">事故记录列表</a></li>
							[/#if]
							[#if access.canDo(user,'safecheck.repair.list')]
							<li class="header"><a id="safecheck.repair.list" onclick="changeTab('safecheck.repair.list','维修记录列表')" title="维修记录列表" target="main">维修记录列表</a></li>
							[/#if]
			...
			<div id="menu_system1" style="padding:8px;">
				 [#if access.canDo(user,'safeauth.dangerous.list')||access.canDo(user,'safeauth.dangerous.managelist')]
				  ...
				  <div>		
					 <ul>
					 	[#if access.canDo(user,'safeauth.dangerous.list')]
					<li><a id="safeauth.dangerous.list" onclick="changeTab('safecheck.safecheck.wfzzhz','整治楼幢汇总表')"title="整治楼幢汇总表"  target="main">整治楼幢汇总表</a></li>
						[/#if]
		...
		<div id="menu_m_healthrecord" style="display:none;">
			<div style="padding-left:10px;font-weight:bold;font-size:14px;padding-right:5px;line-height:36px;height:36px;background-image:url(../resource/images/rebg.jpg);background-repeat:repeat-x;">
		 	健康手册
		 	</div>
			<div id="menu_healthrecord" style="padding:8px;">
				  [#if access.canDo(user,'safecheck.poihealthrecord')]
				  <h2><a>房屋健康手册</a></h2>
				  <div>		
					 <ul>
						<li><a id="safecheck.poihealthrecord" onclick="changeTab('safecheck.poihealthrecord','房屋健康手册地图')" title="房屋健康手册地图" target="main">房屋健康手册地图</a></li>
					</ul>
				  </div>
				  [/#if]
		   </div>
			...
			<div id="menu_member" style="padding:8px;"> 
					...
					<h2><a>人员机构备案管理</a></h2>
						<div>
							...
							<!--<ul>
								[#if access.canDo(user,'safemanage.tBuildingUnitlist')]
								<li><a id="safemanage.tBuildingUnitlist" onclick="changeTab('safemanage.tBuildingUnitlist','管理单位备案')" title="管理单位备案" target="main">管理单位备案</a></li>
								[/#if]
							</ul>-->
							<ul>
								[#if access.canDo(user,'safemanage.tBuildingProjectlist')]
								<li><a id="safemanage.tBuildingProjectlist" onclick="changeTab('safemanage.tBuildingProjectlist','项目备案')" title="项目备案" target="main">项目备案</a></li>
								[/#if]
							</ul>
						</div>
					[/#if]
		...
		<div id="menu_m_login" style="display:none;">
			...
			<div id="menu_login" style="padding:8px;">
				  ...
				  <div>
				  		<ul>
					    	<li><a id="auth.person.userinfo" onclick="changeTab('${property.auth_server_url!''}/portal/auth.person.userinfo','个人中心')" title="个人中心" target="main"><span>个人中心</span></a></li>
					    </ul>
				  </div>
			</div>
		</div>
