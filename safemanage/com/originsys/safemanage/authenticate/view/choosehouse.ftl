      	...
      	// 获取西南角坐标（left，bottom）/房屋点坐标smx,smy
			...
			// 初始化一个弹出窗口，当某个地图要素被选中时会弹出此窗口，用来显示选中地图要素的属性信息
			 ...
			 $("#messfind").bind("click",function() {
			   var a=url+"recorddetail?building_id="+smuserid;					
		     ...
		     $("#messinsert").bind("click",function() {  
		       var b=url+"report.forinsert?smuserid="+smuserid+"&left="+left+"&bottom="+bottom+"&address="+address;
		     		 window.showModalDialog(encodeURI(b),"","dialogWidth:1100px;dialogHeight:600px;center:1;");
	   ...
	   if(view=="新增房屋"){
	     ...
	     function addHouseFinished(editFeaturesEventArgs){
            ...
            if((ids && ids.length > 0) || (resourceInfo && resourceInfo.succeed)) { 
        	 if(window.confirm('保存成功,是否为该房屋录入鉴定信息?')){        	 
             var url="safeauth.report.forinsert1?smuserid="+addHouse.smuserid+"&address="+addHouse.address;
             window.showModalDialog(encodeURI(url),"","dialogWidth:1100px;dialogHeight:600px;center:1;");             	    
             }        
	        }else{
	    	 alert("服务器繁忙,保存失败!");
