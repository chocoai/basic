...
public class SafeCensorUserUpdate extends BaseAction implements IData{
	...
	public void execute(RequestAction ra, HttpServletResponse response)
		...
		if(null!=ra.getParameter("manage_scope")&&null!=ra.getParameter("manage_scope_name")){
		    ...
		    tSafeCensor.setManage_scope(ra.getUser().getOrgcom_id());//String:
		    tSafeCensor.setManage_scope_name(ra.getUser().getOrgcom_name());
		}
...
//		}
		if(null!=ra.getParameter("region")){
		tSafeCensor.setRegion(ra.getParameter("region"));//所属地区		
		}
		else{
		tSafeCensor.setRegion(ra.getUser().getRegion_id());//所属地区			
		}
		String success="0";
		try{
			sc.startTransaction();
